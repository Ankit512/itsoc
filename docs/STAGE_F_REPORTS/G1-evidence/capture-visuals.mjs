 ...
 *
 * Usage:
 *   node docs/STAGE_F_REPORTS/G1-evidence/capture-visuals.mjs \
 *     --expected-commit ac6ca48211f23eda062583a609ed8e4b35c42de3 \
 *     --label after-frozen \
 *     --frozen-report docs/STAGE_F_REPORTS/G1-evidence/frozen-run/report.json
 *
...
function writeEvidence(relativeFile, buffer) {
  const prefix = `docs/STAGE_F_REPORTS/G1-evidence/${LABEL}/`;
...
async function createPage(cdp, baseUrl, route, theme) {
  ...
  await evaluate(cdp, sessionId, `(()=>{
    const prior=document.querySelector("style[data-g1-capture]");if(prior)prior.remove();
    const style=document.createElement("style");style.dataset.g1Capture="true";
...
async function main() {
  ...
  const requiredTreePaths = [
    ...
    "console/serve.py",
    FIXTURE,
    // G1 photographs the nav/shell, so the shell sources are the artefacts this
    // card is evidencing: GUARDRAILS.md 9 requires an evidence card to confirm
    // they exist at HEAD before capturing anything.
  ...
  const workDir = mkdtempSync(path.join(os.tmpdir(), "itsoc-g1-visual-"));
  ...
  try {
      ...
      for (const route of ROUTES) {
        ...
        try {
          ...
          const relativeFile = path.posix.join(
            `docs/STAGE_F_REPORTS/G1-evidence/${LABEL}`, theme, `${route.slug}.png`,
          );
          ...
          if (route.slug === "overview" || route.slug === "incidents") {
            ...
            const relativeRepeat = path.posix.join(
              `docs/STAGE_F_REPORTS/G1-evidence/${LABEL}/stability`, theme, `${route.slug}-repeat.png`,
            );
    ...
    const manifest = {
      ...
      captureSettings: {
        ...
        routeInventorySource: "docs/STAGE_F_REPORTS/G0-evidence/research.md section 6 (route census this harness inherits; G1 adds no route)",
      },
