...
import driftCorpus from "../../../tests/fixtures/audit_drift_corpus.json";
...
describe("AuditTimeline (is-audit-timeline)", () => {
  ...
  it("DRIFT-GUARD: verifies TypeScript port matches committed audit_drift_corpus.json fixture generated by console/audit.py", () => {
    // 1. Single entry vectors: canonical JSON and SHA256 hashes
    for (const vec of driftCorpus.single_vectors) {
      const canonical = canonicalJson(vec.raw);
      expect(canonical).toBe(vec.canonical_json);

      const hash = computeHash(vec.raw);
      expect(hash).toBe(vec.entry_hash);
    }

    // 2. Full chain scenarios: clean, tampered content, severed link, and empty
    for (const [, scen] of Object.entries(driftCorpus.scenarios)) {
      const verification = verifyAuditChain(scen.entries as AuditEntry[]);
      expect(verification.ok).toBe(scen.verification.ok);
      expect(verification.count).toBe(scen.verification.count);

      if (scen.verification.break) {
        expect(verification.break?.index).toBe(scen.verification.break.index);
        expect(verification.break?.reason).toBe(scen.verification.break.reason);
      } else {
        expect(verification.break).toBeNull();
      }

      if (scen.verification.head) {
        expect(verification.head).toBe(scen.verification.head);
      }
    }
  });
});
