#!/usr/bin/env python3
"""
explanation_guard.py — deterministic consistency check between a finding and the
model prose that claims to explain it.

Rules own severity and correlation; the LLM only explains. But prose that names
the wrong host, asserts a different fault than the rule that fired, or invents a
count is worse than no prose at all — it reads as evidence and it is not. This
module is the gate: every explanation is checked against the finding's own
deterministic fields (entities, evidence, summary, predicate, timeline) before
it is shown. No model call is involved; every check is a regex over text the
detector already produced.

verify_explanation(finding, explanation_text) -> {"ok": bool, "reasons": [str]}

Checks:
  (a) grounding — every IP, quoted username, and hostname-shaped token in the
      prose must appear somewhere in the finding's own fields;
  (b) fault type — prose whose vocabulary matches a DIFFERENT rule and never
      touches its own rule's vocabulary is flagged (rule ids and phrasing from
      rule_context.predicate_for and rules_syslog — mirrored here, not invented);
