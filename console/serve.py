...
def resolve_stream_source(value):
    """Resolve a live-stream source safely.

    The browser normally sends STATE.logPath verbatim.  Older SPA state can
    however retain a temporary analysis path after the server has restarted;
    those /tmp/anomaly-console-* paths no longer exist even though the current
    run is valid.  In that case, when the requested path is a local analysis
    temp path and its basename matches the currently loaded run, use the
    server-owned current logPath.  We never accept an arbitrary filesystem
    ...
    if current:
        ...
        if value == str(current_path) and current_path.exists():
            return current_path

        # Recover from stale SPA state after a server restart.  Only allow
        # paths that look like our own temporary analysis paths, and only when
        # the basename matches the current run's source.
        try:
            requested = Path(value)
            stale_tmp = (
                requested.name == current_path.name
                and requested.parent.name.startswith("analysis-")
                and requested.parent.parent.name.startswith("anomaly-console-")
...
class ConsoleHandler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        elif path == "/api/oem/templates":
            self._json({"templates": ti_oem.templates()})
